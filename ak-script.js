// Aalborg Kommune Project - Quantum Reality Script

class AalborgKommuneQuantumReality {
    constructor() {
        this.currentReality = 'aalborg';
        this.isActive = false;
        this.quantumState = 'municipal';
        this.init();
    }
    ...
    activateReality() {
        this.isActive = true;
        console.log('Aalborg Kommune Quantum Reality activated');
    ...
    activatePortal(portal) {
        ...
        const portalVideo = portal.querySelector('.portal-video');
        const portalMeta = portal.querySelector('.portal-meta');
        
        portal.style.transform = 'translateY(-5px) scale(1.02)';
        portal.style.boxShadow = '0 20px 40px rgba(52,138,199,0.2)';
        ...
        if (portalVideo) {
            portalVideo.style.transform = 'scale(1.1)';
        }
    ...
    deactivatePortal(portal) {
        ...
        if (portalVideo) {
            portalVideo.style.transform = 'scale(1)';
        }
    ...
    // Video Portal Enhancement
    setupVideoPortal() {
        const videoPortal = document.querySelector('.video-portal');
        const video = videoPortal?.querySelector('.portal-video');
        
        if (!videoPortal || !video) return;

        // Add play/pause on click functionality
        videoPortal.addEventListener('click', (e) => {
            // Prevent lightbox trigger for video portal
            e.stopPropagation();
        ...
        // Add visual indicator for video state
        const createVideoIndicator = () => {
            ...
            indicator.style.cssText = `
                ...
                z-index: 10;
            `;
            videoPortal.appendChild(indicator);
            return indicator;
        };
        ...
        // Show indicator on hover when paused
        videoPortal.addEventListener('mouseenter', () => {
            if (video.paused) {
                indicator.style.opacity = '1';
            }
        });

        videoPortal.addEventListener('mouseleave', () => {
            if (video.paused) {
    ...
    setupNavEffects() {
        ...
        navLinks.forEach(link => {
            link.addEventListener('mouseenter', () => {
                ...
                link.style.color = 'var(--ak-primary)';
            });
        ...
        navPortals.forEach(portal => {
            portal.addEventListener('mouseenter', () => {
                ...
                portal.style.borderColor = 'var(--ak-primary)';
            });
            
            portal.addEventListener('mouseleave', () => {
                portal.style.transform = 'translateY(0)';
                portal.style.borderColor = 'rgba(52,138,199,0.3)';
            });
        });
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('Aalborg Kommune Quantum Reality - Initializing...');
    
    // Initialize main reality system
    const akReality = new AalborgKommuneQuantumReality();
    
    // Setup value icons after reality is initialized
    akReality.setupValueIcons();
    ...
    console.log('Aalborg Kommune Quantum Reality - Fully activated!');
});
...
document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
        ...
        // Don't auto-resume videos, let user control them
    }
});

// Export for potential use in other modules
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { AalborgKommuneQuantumReality, QuantumNavigation };
}
